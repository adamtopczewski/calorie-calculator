...
export class MealsService {
  ...
  getNutr(){
    let sugar = this.totalNutrients.map(a => a.SUGAR.quantity).reduce((ac, a) => ac += a).toFixed() ;
    let protein = this.totalNutrients.map(a => a.PROCNT.quantity).reduce((ac, a) => ac += a).toFixed();
    let fat = this.totalNutrients.map(a => a.FAT.quantity).reduce((ac, a) => ac += a).toFixed();
    let calories = this.totalNutrients.map(a => a.ENERC_KCAL.quantity).reduce((ac, a) => ac += a).toFixed();
    return {
      sugar,
      protein,
      fat,
